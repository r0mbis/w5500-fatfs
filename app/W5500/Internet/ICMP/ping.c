...
uint8_t ping_auto(uint8_t s, uint8_t* addr) {
    ...
    for(i = 0; i <= 3; i++) {
        ...
        log_i(ETH_TAG, "Socket number to ping: %02X", sr);
        ...
        case SOCK_CLOSED:
            ...
            if((ret = socket(s, Sn_MR_IPRAW, 3000, 0)) !=
               s) { // open the SOCKET with IPRAW mode, if fail then Error
                log_i(ETH_TAG, "  socket %d fail %d", s, ret);
#ifdef PING_DEBUG
                return SOCKET_ERROR;
#endif
            }
        ...
        case SOCK_IPRAW:
            ...
            while(1) 
            {
                ...
                {
                    log_i(ETH_TAG, "Ping request Time out.");
                    cnt = 0;
                    break;
                } 
                else 
...
#ifdef PING_DEBUG
        if(req >= 3) {
            log_i(ETH_TAG, "Ping Request = %d, PING_Reply = %d", req, rep);
...
uint8_t ping_count(uint8_t s, uint16_t pCount, uint8_t* addr) {
    ...
    for(i = 0; i < pCount + 1; i++) {
        if(i != 0) {
            /* Output count number */
            log_i(ETH_TAG, "No.%d", (i - 1));
        }

        uint8_t sr = getSn_SR(s);
        log_i(ETH_TAG, "SR: %d", sr);
        switch(sr) {
        case SOCK_CLOSED:
            close(s);
            // close the SOCKET
        ...
        case SOCK_IPRAW:
            ...
            while(1) {
                ...
                /* wait_time for 2 seconds, Break on fail*/
                if((cnt > 100)) {
                    log_i(ETH_TAG, "Request Time out.");
                    cnt = 0;
                    break;
                } else {
                    cnt++;
...
uint8_t ping_request(uint8_t s, uint8_t* addr) {
    ...
    if(sendto(s, (uint8_t*)&PingRequest, sizeof(PingRequest), addr, 3000) ==
       0) { // Send Ping-Request to the specified peer.
        log_i(ETH_TAG, " Fail to send ping-reply packet ");
    } else {
        log_i(
            ETH_TAG,
...
uint8_t ping_reply(uint8_t s, uint8_t* addr, uint16_t rlen) {
    ...
    if(data_buf[0] == PING_REPLY) 
    {
        ...
        {
            log_i(ETH_TAG, "tmp_checksum = %x", tmp_checksum);
        } 
        else 
        {
    ...
    {
        uint16_t checksum_req = 0;
        checksum_req = (data_buf[2] << 8) | data_buf[3];
        PingReply.Code = 0;
        PingReply.Type = PING_REPLY;
        PingReply.CheckSum = 0; //(data_buf[3] << 8) | data_buf[2];
        data_buf[2] = 0;
        data_buf[3] = 0;
        PingReply.ID = (data_buf[5] << 8) | data_buf[4];
        PingReply.SeqNum = (data_buf[7] << 8) | data_buf[6];
        ...
        /* check Checksum of Ping Reply */
        tmp_checksum = checksum((uint8_t*)data_buf, len);//PingReply.CheckSum;
        //PingReply.CheckSum = 0;
        PingReply.CheckSum = htons(checksum((uint8_t*)&PingReply, len));

        if(tmp_checksum != checksum_req) 
        {
            log_i(ETH_TAG,
                "CheckSum is in no correct %x shold be %x",
                tmp_checksum,
                checksum_req);
        } 
        else 
        {
            log_i(ETH_TAG, "Checksum is correct");
        }
